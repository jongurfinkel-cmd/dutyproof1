...
export async function POST(req: NextRequest) {
  try {
    const limited = rateLimit(req, { limit: 10, windowSec: 60, prefix: 'closeout-upload' })
    ...
    const path = `closeout/${user.id}/${Date.now()}.${ext}`
    ...
    if (uploadError) {
      console.error('Closeout upload error:', uploadError)
    ...
    console.error('Closeout upload error:', err)
